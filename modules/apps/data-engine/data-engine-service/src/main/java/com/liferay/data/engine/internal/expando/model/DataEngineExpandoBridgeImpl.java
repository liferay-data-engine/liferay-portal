...
public class DataEngineExpandoBridgeImpl implements ExpandoBridge {
	...
	public Serializable getAttribute(String name, boolean secure) {
		...
		try {
			...
			return (Serializable)dataRecordValues.get(_findDataRecordValueKey(dataRecordValues, name));
		}
		catch (Exception exception) {
		...
		try {
			...
			if (ddlRecord == null) {
				...
				dataRecord.setDataRecordValues(
					HashMapBuilder.<String, Object>put(
						DataRecordValueKeyUtil.createDataRecordValueKey(name,
							StringUtil.randomString(), StringPool.BLANK, 0), value
					).build());
	...
	private String _findDataRecordValueKey(
		...
		for (Map.Entry<String, Object> entry: dataRecordValues.entrySet()) {
			String[] dataRecordValueKeyParts = DDMFormFieldParameterNameUtil.getLastDDMFormFieldParameterNameParts(entry.getKey());

			String fieldName = dataRecordValueKeyParts[DDMFormFieldParameterNameUtil.DDM_FORM_FIELD_NAME_INDEX];
	...
	protected DataDefinitionField createDataDefinitionField(
		...
		DDMFormField ddmFormField = settingsDDMFormFields.get("dataType");
